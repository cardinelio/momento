...
class MainWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.button_funnynoise = QPushButton("Веселая кнопка", self)
        ...
        self.button_funnynoise.clicked.connect(self.funnynoise)
        self.init_ui()
        self.create_db()
        self.load_tasks()
        ...
        #self.tasks_list.doubleClicked.connect(self.change_status)

        self.tasks_list.itemDoubleClicked.connect(self.change_status)
    ...
    def init_ui(self):
        ...
        hbox = QHBoxLayout()
        hbox.addWidget(self.button_funnynoise)
    ...
    def funnynoise(self):
        self.sound = QSound('smb_1-up.wav', self)
        self.sound.play()
    def load_tasks(self):
        query = QSqlQuery()
        query.exec(
    ...
    def change_status(self):
        ...
        if result == QMessageBox.No:
            query = QSqlQuery()
            query.exec(
                f"""UPDATE tasks SET active = 1 WHERE id={task_id};""")
            self.load_tasks()
...
def show_message():
    msg = QMessageBox()
    msg.setWindowTitle('Здравствуйте!')
    msg.setText('Перед тем как вы воспользуетесь этой программой, хочу поздравить вас с Новым 2023 Годом!')
